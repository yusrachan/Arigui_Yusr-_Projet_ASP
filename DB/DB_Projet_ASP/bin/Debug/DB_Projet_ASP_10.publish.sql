...
GO
/*
La colonne [dbo].[Produit].[Id_Med] est en cours de suppression, des données risquent d'être perdues.
*/
...
GO
ALTER TABLE [dbo].[Produit] DROP COLUMN [Id_Med];


GO
PRINT N'Modification de Procédure [dbo].[SP_Produit_GetAll]...';


GO
ALTER PROCEDURE [dbo].[SP_Produit_GetAll]
AS
	SELECT [Id_Prod],
			[nom],
			[description],
	...
	FROM [Produit]
GO
PRINT N'Modification de Procédure [dbo].[SP_Produit_GetById]...';


GO
ALTER PROCEDURE [dbo].[SP_Produit_GetById]
	@id_prod INT
AS
	SELECT [Id_Prod],
			[nom],
...
GO
PRINT N'Modification de Procédure [dbo].[SP_Produit_Insert]...';


GO
ALTER PROCEDURE [dbo].[SP_Produit_Insert]
	@no NVARCHAR(64),
	@desc NVARCHAR(MAX),
	@pri DECIMAL(10,2),
	@cr_ec NVARCHAR(64),
...
GO
PRINT N'Modification de Procédure [dbo].[SP_Produit_Update]...';


GO
ALTER PROCEDURE [dbo].[SP_Produit_Update]
	@id_prod INT,
	@no NVARCHAR(64),
	@desc NVARCHAR(MAX),
	@pri DECIMAL(10,2),
...
EXEC SP_Produit_Insert 'iPhone 14', NULL, 599.99, NULL, 1, 1;
EXEC SP_Produit_Insert 'Moi, Yusrâ', 'Ce livre raconte la vie d’une femme formidable qui est Arigui Yusrâ. Entre rebondissements, émotions et inspirations, vous ressortirez de son histoire avec de l’admiration et du respect pour cette jeune femme.', 99.99, 'Papier recyclé', 2, 2;
EXEC SP_Produit_Insert 'TV LED Samsung', 'The Frame, la TV ArtyFaites entrer l’art chez vousThe Frame est conçu pour sublimer votre intérieur qu’il soit allumé ou éteint. Quand il est allumé, The Frame offre une qualité d’image exceptionnelle grâce à la technologie QLED Full HD.', 99.99, NULL, 3, 3;
